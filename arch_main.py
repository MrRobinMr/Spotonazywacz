...
def fileN(kat, fre, nam, sta, end, dur, addname, corner):
    ...
    list_file = list()
    for i in fp:
        list_file.append(p.Path(i.name))
...
def print_hi():
    ...
    button_join = tk.Button(text="Połącz zdjęcia", width=10, height=1, bg="blue", font=4, fg="white", command=lambda: Connect(kat,fre,nam,sta,end,dur,AddName,checkbox_var))
    button_join.pack()
    button.pack()
    window.mainloop()
